...
<body>
	...
	<div class=" " id="" style="margin-top: 200px; margin-bottom: 200px;"
		...
		function messageSend() {
			...
			$(function() {
				$("#messageList>tbody>tr>td").click(
						function() {
							location.href = 'messageDetail.msg?msgno='
			...
			function deleteMessage() {
				var msgCheck = document.getElementsByName('msgCheck');

				var cnt = $("input[name='msgCheck']:checked").length;
				var arr = new Array();

				var result = 1;

				for (var i = 0; i < msgCheck.length; i++) {
					if (msgCheck[i].checked == true) {
						arr[cnt] = msgCheck[i].value;
						cnt++;
					}
				}

				if (cnt == 0) {
					alert("삭제하실 메세지를 선택해주세요.");
				} else {
					if (confirm("선택하신 메세지를 삭제하시겠습니까?")) {
						$.ajax({
							type : "POST",
							url : "${pageContext.request.contextPath}/checkedMessageDelete.msg",
							data : {
								arr
							},
							dataType : "json",
			...
			function reportMessage() {
				...
				if (cnt == 0) {
					alert("신고하실 메세지를 선택해주세요.");
				} else {
					if (confirm("선택하신 메세지를 신고하시겠습니까?")) {
						$.ajax({
							type : "POST",
							url : "${pageContext.request.contextPath}/checkedMessageReport.msg",
							data : {
								arr
							},
							dataType : "json",
		...
		<div class="">
			<button type="button" class="btn btn-secondary"
				onclick="deleteMessage();">삭제</button>
			<button type="button" class="btn btn-danger"
				onclick="reportMessage();">신고</button>
		</div>
		...
		<div id="pagingArea" class="">
			<ul class="pagination justify-content-center"
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link"
							href="messageList.msg?cpage=${pi.currentPage-1}">&laquo;</a></li>
					</c:otherwise>
				</c:choose>
				...
				<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}">
					...
					<li class="page-item"><a class="page-link"
						href="messageList.msg?cpage=${p }">${p}</a></li>
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link"
							href="messageList.msg?cpage=${pi.currentPage+1}">&raquo;</a></li>
					</c:otherwise>
				</c:choose>
			</ul>
		</div>
